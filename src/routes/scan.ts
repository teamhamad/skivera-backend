...
router.post("/face", (req: Request, res: Response) => {
  ...
  const skinType: SkinType =
    drynessSignal > 0.7 ? "Dry" : oilSignal > 0.72 ? "Oily" : score < 65 ? "Sensitive" : oilSignal > 0.52 && drynessSignal > 0.52 ? "Combination" : "Balanced";
...
router.get("/test-token", (req, res) => { res.json({ message: "Auth bypassed for testing" }); });
